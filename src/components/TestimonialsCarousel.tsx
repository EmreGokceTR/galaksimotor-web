            ...
            {list.map((_, i) => (
              <button
                ...
                className={`h-1.5 rounded-full transition-all ${
                  i === index ? "w-8 bg-brand-yellow" : "w-1.5 bg-white/20 hover:bg-white/40"
                }`}
              />
            ))}
          </div>
...
function Stars({ rating }: { rating: number }) {
  return (
    <div className="flex gap-0.5">
      {[1, 2, 3, 4, 5].map((n) => (
        <svg key={n} viewBox="0 0 24 24" className="h-5 w-5"
          fill={n <= rating ? "#FFD700" : "none"} stroke="#FFD700" strokeWidth={1.5} strokeLinejoin="round">
