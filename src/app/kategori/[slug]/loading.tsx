export default function KategoriLoading() {
  return (
    <div className="mx-auto max-w-7xl px-6 py-10">
      {/* Header skeleton */}
      <div className="mb-2 h-8 w-64 animate-shimmer rounded-lg bg-white/[0.06]" />
      <div className="mb-8 h-4 w-96 max-w-full animate-shimmer rounded bg-white/[0.06]" />

      {/* Filter bar skeleton */}
      <div className="mb-6 flex gap-3 overflow-hidden">
        {[1, 2, 3, 4].map((i) => (
          <div key={i} className="h-9 w-24 shrink-0 animate-shimmer rounded-full bg-white/[0.06]" />
        ))}
      </div>
