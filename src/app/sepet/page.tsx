    ...
    <div className="mx-auto max-w-7xl px-6 py-14">
      <header className="mb-10">
        ...
        <h1 className="mt-2 text-3xl font-bold tracking-tight text-white md:text-4xl">
          {items.length === 0
            ? "Sepetin boş"
            : `${items.reduce((a, i) => a + i.quantity, 0)} ürün hazır`}
        </h1>
      </header>
        ...
        <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_360px]">
          ...
          <ul className="space-y-3">
              ...
              {items.map((it) => (
                ...
                >
                  ...
                  <div className="min-w-0">
                    ...
                    {it.variantLabel && (
                      <p className="mt-0.5 text-xs text-white/50">{it.variantLabel}</p>
                    )}
                    <p className="mt-0.5 text-xs text-white/30">SKU · {it.sku}</p>
                    <div className="mt-3 flex items-center gap-3">
                      <div className="flex items-center rounded-full border border-white/15 bg-white/5">
                        ...
                        <input
                          ...
                          value={it.quantity}
                          onChange={(e) =>
                            setQty(it.key, parseInt(e.target.value || "1", 10))
                          }
            ...
            <div className="flex items-center justify-between pt-2 text-sm">
              <button
                onClick={() => {
                  if (confirm("Sepeti tamamen temizlemek istiyor musun?")) clear();
                }}
            ...
            <div className="overflow-hidden rounded-2xl border border-white/10 bg-white/[0.04] backdrop-blur-md">
              <div className="border-b border-white/10 bg-gradient-to-br from-brand-yellow/10 via-transparent to-brand-yellow/5 p-5">
                ...
                <p className="mt-1 text-xs text-white/50">
                  Kargo bilgisi sonraki adımda hesaplanır.
                </p>
              </div>
              <div className="space-y-3 p-5">
                ...
                <Row
                  label="Kargo"
                  value={<span className="text-emerald-300">Hesaplanacak</span>}
                />
