...
function LeftPanel() {
  ...
  return (
    <div
      ...
      style={{
        background:
          "linear-gradient(145deg, #080808 0%, #111111 60%, #0c0c0c 100%)",
      }}
    >
      <div
      ...
      <div
        ...
        style={{
          background:
            "radial-gradient(circle at 0% 0%, rgba(255,215,0,0.07) 0%, transparent 70%)",
        }}
      />
      <div
        ...
        style={{
          background:
            "radial-gradient(circle at 100% 100%, rgba(255,215,0,0.05) 0%, transparent 70%)",
        }}
      />
      <motion.div
        ...
        style={{
          ...
          height: 420,
          background:
            "radial-gradient(circle, rgba(255,215,0,0.045) 0%, transparent 68%)",
          top: "50%",
          left: "50%",
          x: "-50%",
          y: "-50%",
      ...
      >
        <p className="text-[11px] tracking-[0.55em] text-brand-yellow/55 uppercase mb-3 font-medium">
          Galaksi Motor
        </p>
        <div className="h-px w-20 mx-auto bg-gradient-to-r from-transparent via-brand-yellow/35 to-transparent mb-3" />
        <p className="text-[13px] text-white/30 tracking-[0.18em] font-light">
          Motosiklet Tutkusu · Premium Kalite
        </p>
      </motion.div>
      {dots.map(({ style, delay }, i) => (
        <motion.div
          key={i}
          className="absolute w-1 h-1 rounded-full bg-brand-yellow"
...
function LoginForm({
  ...
  async function handleCredentials(e: FormEvent) {
    ...
    try {
      ...
      if (res?.ok && !res.error) {
        // Hard navigation — middleware/server session güncel okusun.
  ...
  const container: Variants = {
    ...
    visible: {
      opacity: 1,
      transition: { staggerChildren: 0.07, delayChildren: 0.15 } as never,
    },
  };
  const item: Variants = {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <p className="text-[11px] tracking-[0.4em] text-brand-yellow/60 uppercase mb-2 font-medium">
            Galaksi Motor
          </p>
          <h1 className="text-3xl font-bold text-white leading-tight">
            ...
            <span
              ...
              style={{ textShadow: "0 0 20px rgba(255,215,0,0.35)" }}
            >
              hoş geldin.
            </span>
          </h1>
          <p className="mt-2 text-sm text-white/40">
            Hesabına giriş yap, yolculuğuna devam et.
          </p>
        </motion.div>
        ...
        >
          ...
          <Link
            href="/kayit"
            className="text-brand-yellow/80 hover:text-brand-yellow transition-colors duration-150 font-medium"
          >
            Kayıt olun
          </Link>
        </motion.p>
